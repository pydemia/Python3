...
| ```os.access(path, mode)``` | return ```True/False``` if <mode> can operate in <path>  |    ```os.access('~/python', 'F_OK')```  
<br/> 'F_OK': check the path exists  <br/> 'R_OK': check `read` permission  <br/> 'W_OK': check `write` permission  <br/> 'X_OK': check `execute` permission  <br/> 'R_OK': check `read` permission |

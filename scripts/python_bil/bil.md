...
|```t1 = t2 * f``` </br> or ```t1 = f * t2```	 | Delta multiplied by a ```float```. </br> The result is rounded to the nearest multiple of ```timedelta.resolution``` using round-half-to-even. |
|```f = t2 / t3```	 | Division (3) of ```t2``` by ```t3```. </br> Returns a ```float``` object. |
|```t1 = t2 / f``` </br> or ```t1 = t2 / i```	 | Delta divided by a ```float``` or an ```int```. </br> The result is rounded to the nearest multiple of ```timedelta.resolution``` using round-half-to-even. |
